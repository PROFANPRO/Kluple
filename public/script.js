...
function connectByWalletInfo(w) {
  try {
    if (
      TonConnectSDK.isWalletInfoCurrentlyEmbedded?.(w) ||
      TonConnectSDK.isWalletInfoCurrentlyInjected?.(w)
    ) {
...
async function updateBalanceByPublicAPIs(friendlyAddress) {
  try {
    const r = await fetch(
      `https://tonapi.io/v2/accounts/${encodeURIComponent(friendlyAddress)}`
    );
    if (r.ok) {
    ...
    setTimeout(async () => {
      ...
      try {
        const verifyResp = await fetch(
          `/api/verify-deposit?userAddress=${friendlyAddress}`
        );
        const verifyData = await verifyResp.json();

        if (verifyResp.ok) {
          balanceDisplay.textContent = `${verifyData.totalDepositedTON.toFixed(
            4
          )} TON`;
          alert(
            `Зачислено: ${verifyData.totalDepositedTON.toFixed(4)} TON`
          );
        } else {
...
// === Глобализируем функции, чтобы их видел HTML ===
